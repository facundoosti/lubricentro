class Api::V1::ConversationsController < ApplicationController
  before_action :set_conversation, only: [ :show, :assign_human, :archive, :mark_as_supplier ]
  ...
  # PATCH /api/v1/conversations/:id/mark_as_supplier
  def mark_as_supplier
    @conversation.update!(status: "supplier")

    SupplierPhone.find_or_create_by!(phone: @conversation.whatsapp_phone) do |sp|
      sp.company_name = params[:company_name]
      sp.notes        = params[:notes]
    end

    broadcast_status_update(@conversation)
    data = ConversationSerializer.render_as_hash(@conversation, root: :conversation)
    render_json(data, message: "Conversación marcada como proveedor")
  end
